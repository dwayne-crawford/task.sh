...
router.get('/date', dateRangeValidation, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (format !== 'json') {
      const converted = convertExportData(exportResponse, format, `date-${query.date_from}-to-${query.date_to}-export`);
      res.setHeader('Content-Type', converted.mimeType);
      res.setHeader('Content-Disposition', `attachment; filename="date-${query.date_from}-to-${query.date_to}-export.${converted.extension}"`);
...
// GET /api/export/projects - List all projects (must come before /:id route)
...
router.get('/tasks/complete', statusExportValidation, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const taskList = getTaskListForUser(req.user!.id);
    ...
    if (format !== 'json') {
      const converted = convertExportData(exportResponse, format, 'completed-tasks-export');
      res.setHeader('Content-Type', converted.mimeType);
      res.setHeader('Content-Disposition', `attachment; filename="completed-tasks-export.${converted.extension}"`);
      res.send(converted.content);
    } else {
      res.setHeader('Content-Type', 'application/json');
...
router.get('/tasks/incomplete', statusExportValidation, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const taskList = getTaskListForUser(req.user!.id);
    ...
    if (format !== 'json') {
      const converted = convertExportData(exportResponse, format, 'incomplete-tasks-export');
      res.setHeader('Content-Type', converted.mimeType);
      res.setHeader('Content-Disposition', `attachment; filename="incomplete-tasks-export.${converted.extension}"`);
      res.send(converted.content);
    } else {
      res.setHeader('Content-Type', 'application/json');
...
router.post('/bulk', bulkExportValidation, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Handle different formats
    const format = bulkRequest.format || 'json';
    
    if (format !== 'json') {
      const converted = convertExportData(bulkResponse, format, `bulk-export-${Date.now()}`);
      res.setHeader('Content-Type', converted.mimeType);
      res.setHeader('Content-Disposition', `attachment; filename="bulk-export-${Date.now()}.${converted.extension}"`);
      res.send(converted.content);
    } else {
      res.setHeader('Content-Type', 'application/json');
